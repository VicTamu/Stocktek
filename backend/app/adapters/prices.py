...
def fetch_price_bars(
    ...
    """Fetch daily bars through yfinance.

    The adapter returns plain dictionaries so the rest of the app can later swap
    in a licensed data provider without changing analysis code.
    """
    if not symbols:
        return [], []
    ...
    try:
        ...
        raise PriceAdapterError("yfinance is not installed") from exc
